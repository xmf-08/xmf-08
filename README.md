# 👋 Hi, I'm Mukhammadyusuf Khakimov

## 🧠 Pre-Senior Android Developer | AI Researcher | Future Tech Leader

---

## 🚀 Who I Am

I am a highly motivated 17-year-old developer, public speaker, researcher, and community leader from Margilan, Uzbekistan. Currently attending School No. 2 with a GPA of **4.9**, I am known for setting an example for my peers through both academic excellence and active community contribution. I once won a spot at the Presidential School of Fergana but chose to stay at my beloved school to continue leading and inspiring my classmates.

## 🧑‍💻 Tech Skills & Projects

- **Android Development:** Over 100+ apps created in one year, ranging from professional tools to experimental prototypes.
- **Languages & Tools:** Kotlin, Java, Python, Firebase, Jetpack Compose, AI API integrations, Git
- **Notable Projects:**
  - **MediPulse** – A custom-built pharmacy management app, currently deployed in my family's pharmacy and sold to two additional pharmacies for $500 each.
  - **Real Estate CRM** – A full-featured app built during a hackathon, now used by a real estate manager.
  - **Telegram-like Chat System** – Developed early in my career as a practical intro to real-time communication apps.
  - **KNews App** – A custom news aggregation app built using live APIs.
  - **Children’s Educational Games** – Flag quiz games and general knowledge games designed for young learners.
  - **Website regarding the Philophy** [it's available here](https://xmf-08.github.io/Philosophy/sophists_to_modern_philosophy_long_form_blog_html.html)

- **Certificates:**
  - Junior Android Developer (now self-taught to pre-senior level)
  - Certified Computer Literacy Instructor *("Kompyuter savodxonligi")*

---

## 🧪 AI Research & Cybersecurity

- Conducting a research project on **AI agency vs. human reasoning**
- Working with **7 AI models** to analyze and compare their responses against real human answers
- **Collected 100+ human response samples** and preparing an academic paper with detailed methodology and analysis
- Planning to publish soon – [**Link coming soon**]

## 🎯 Future Vision

My long-term ambition is to work at the intersection of **ethical AI** and **robotics**, contributing to the development of intelligent systems that serve humanity responsibly and effectively.

---

## 🗣 Public Speaking & Leadership

- Participant in the elite **Hogwarts camp** - official 7-day program of New Uzbekistan University
- 120$ scolarship(fully covered) winner among 500+ strong applicants

- 🧭 Values-Driven Decisions
🎓 Accepted into ENG4IT (a top-tier English for IT program run by the U.S. Embassy)
  ⤷ Chose to step back after selection, prioritizing personal learning goals and already-acquired experience
  ⤷ A decision grounded in clarity of purpose, not lack of ability
  
- **Debate Achievements:**
  - **Best Debater of Margilan and Fergana** – Officially awarded by Ibrat Debate (Uzbekistan’s leading debate institution)
  - **Judge and Organizer** at Fergana City Debate sessions
  - Participant in the elite **Ibrat Debate Camp**, where I refined my debate skills over a 7-day immersion

- **Speaking Club Coordinator:**
  - Co-leader of the **Kevin’s Academy Speaking Club** with 50+ weekly attendees
  - Achieved **IELTS 7.5** in just 4–5 months, aiming for **8.5** soon
  
- **MUN Recognition:**
  - **Best Speaker Award** at Turan MUN representing the USA in ECOSOC Committee

- **Workshops & Contributions:**
  - Led healthcare & environmental preservation campaigns
  - Produced and starred in short films and documentaries
  - Created and presented educational presentations and visual materials

---

## 🧑‍🎓 Academics

- **GPA:** 4.9 (Top performer in STEM and health science subjects)
- **IELTS:** 7.5 (Target: 8.5+ – Upcoming)
- **SAT:** 1470 mock (790 Math, 680 English) – **Target: 1500+ – Real SAT Coming Soon**
- **Olympiads:** Participant in national-level physics and English competitions

---

## 🧠 Creative & Digital Skills

- **Web Development:** Proficient in HTML/CSS – building websites without AI tools
- **Animation:** Created an original animation using Scratch and other tools
- **Graphic Design:** Strong eye for clean, professional UI and content visuals

---

## 📚 Extracurricular Leadership

- Founder of an **eco-health awareness group** on Telegram
- Actively documenting work on healthcare advocacy and environmental action
- Articles and videos posted on [my Telegram Channel](https://t.me/yaxshi_amalllar)

---

## 💬 Connect With Me

📫 **Email:** [muhammadyusuf020208@gmail.com](mailto:muhammadyusuf020208@gmail.com)  
🌐 **GitHub:** [xmf-08](https://github.com/xmf-08)  
🗨 **LinkedIn:** [mukhammadyusuf-xakimov](https://www.linkedin.com/in/mukhammadyusuf-khakimov-21a195250)  
💬 **Telegram (Personal):** [life_is_comedia](https://t.me/life_is_comedia)  
📢 **Telegram (Channel):** [this_is_a_real_me](https://t.me/this_is_a_real_me)

---

## 📌 Location

Chustiy 71, Margilan 150100, Uzbekistan

---

**Let’s build the future — ethically, intelligently, and together.**
